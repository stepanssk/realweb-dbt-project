...
WITH 
--выбираем из stg_af_installs установки по условиям is_realweb=TRUE, source!='other', is_ret_campaign=FALSE
installs AS (
    SELECT 
        date,
        campaign_name,
...
--группируем installs так, чтобы в дальнейшем было удобно сджойнить grouped с таблицей по всем источникам sources
grouped AS (
    SELECT
        date,
        campaign_name,
...
-- из таблиц по каждому источнику берем нужные данные, с соответствующими условиями (is_realweb=TRUE, is_ret_campaign=FALSE), приводя к виду финальной таблицы
--в некоторых источниках отсутствуют столбец с установками, добавляем его путем использования NULL installs (для возможности использования функции UNION ALL)
