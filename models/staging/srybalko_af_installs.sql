-- Модель представляет собой SQL-код,
-- по результатам работы которого должна получиться целевая таблица:
...
-- а если нет - то из stg_af_intalls (это требование заказчика, такое бывает).
--
...
WITH
    -- из stg_af_install берем только кампании Риалвеба с известным источником и только User Acquisition
    installs AS (
        SELECT
            date,
            campaign_name,
    ...
    -- формируем срезы для разбивки по дате, кампании, группе объявления, платформе и источнику
    -- и считаем installs, т к не во всех рекламных кабинетах они есть в данных
    slices AS (
        SELECT
            date,
            campaign_name,
    ...
    -- далее из всех источников забираем нужные данные в таблицы с одинаковой структурой
    -- date, campaign_name, adset_name, platform, source, clicks, costs, installs, impressions
    -- только кампании Риалвеба и только User Acquisition
    -- чтобы потом объединить все данные в одну таблицу
    facebook AS (
         SELECT
             date,
             campaign_name,
